...
Daily digest отправляется **только** в тестовый канал. Переменная **TELEGRAM_TEST_CHANNEL_ID** задаёт чат/канал, куда уходят сообщения. Основной канал (**TELEGRAM_CHAT_ID**) для дайджеста не используется.

Для отправки нужен **TELEGRAM_BOT_TOKEN**. Bridge должен быть запущен (локально или в Docker).
