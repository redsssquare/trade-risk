#!/usr/bin/env node
/**
 * Импортирует или обновляет workflow "Daily Digest" в n8n из файла
 * n8n-daily-digest-workflow.json и активирует его.
 * Требует N8N_API_KEY в .env (n8n: Settings → API).
 *
 * Локально:  node scripts/push-daily-digest-workflow.js
 * Docker:   docker compose run --rm --entrypoint node -e N8N_BASE_URL=http://n8n:5678 -v "$(pwd):/app" -w /app bridge scripts/push-daily-digest-workflow.js
 */
...
const WORKFLOW_FILE = "n8n-daily-digest-workflow.json";
